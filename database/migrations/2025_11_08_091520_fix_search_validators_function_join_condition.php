        ...
        DB::statement("
            ...
            BEGIN
                ...
                HAVING (p_filter_type IS NULL OR p_filter_type = '' OR p_filter_type = 'all' 
                         ...
                         OR (p_filter_type = 'favorite' AND p_user_id IS NOT NULL AND bool_or(v2u.type = 'favorite' AND v2u.user_id = p_user_id))
                         OR (p_filter_type = 'compare' AND p_user_id IS NOT NULL AND bool_or(v2u.type = 'compare' AND v2u.user_id = p_user_id))
                         OR (p_filter_type = 'blocked' AND p_user_id IS NOT NULL AND bool_or(v2u.type = 'blocked' AND v2u.user_id = p_user_id))
                         OR (p_filter_type = 'notice' AND p_user_id IS NOT NULL AND bool_or(v2u.type = 'notice' AND v2u.user_id = p_user_id)))
                ORDER BY 
                    CASE 
        ...
        DB::statement("
            ...
            BEGIN
                ...
                HAVING (p_filter_type IS NULL OR p_filter_type = '' OR p_filter_type = 'all' 
                         OR (p_filter_type = 'highlight' AND bool_or(v.is_highlighted)) 
                         OR (p_filter_type = 'top' AND bool_or(v.is_top))
                         OR (p_filter_type = 'favorite' AND p_user_id IS NOT NULL AND bool_or(v2u.type = 'favorite'))
                         OR (p_filter_type = 'compare' AND p_user_id IS NOT NULL AND bool_or(v2u.type = 'compare'))
                         OR (p_filter_type = 'blocked' AND p_user_id IS NOT NULL AND bool_or(v2u.type = 'blocked'))
                         OR (p_filter_type = 'notice' AND p_user_id IS NOT NULL AND bool_or(v2u.type = 'notice')))
                ORDER BY 
                    CASE 
                        WHEN p_sort_column = 'spy_rank' THEN v.spy_rank
                        WHEN p_sort_column = 'tvc_rank' THEN v.tvc_rank
