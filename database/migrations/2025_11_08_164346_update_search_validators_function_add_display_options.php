...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement('DROP FUNCTION IF EXISTS data.search_validators(text, text, bigint, text, integer, integer)');
        
        // Create the updated function with display options
        DB::statement("
            CREATE OR REPLACE FUNCTION data.search_validators(
                p_search_term text, 
                p_filter_type text, 
                p_user_id bigint, 
                p_sort_column text, 
    ...
    public function down(): void
    {
        ...
        DB::statement('DROP FUNCTION IF EXISTS data.search_validators(text, text, bigint, text, integer, integer, boolean, boolean, boolean, boolean, boolean, bigint[])');
	...
	' from COALESCE(p_search_term, ''))) || '%'))
                    AND (p_filter_type IS NULL OR p_filter_type = '' OR p_filter_type = 'all' 
                         ...
                         OR (p_filter_type = 'favorite' AND v2u.id IS NOT NULL AND v2u.type = 'favorite')
                         OR (p_filter_type = 'compare' AND v2u.id IS NOT NULL AND v2u.type = 'compare')
                         OR (p_filter_type = 'blocked' AND v2u.id IS NOT NULL AND v2u.type = 'blocked'))
                GROUP BY 
                    v.id, v.vote_pubkey, v.node_pubkey, v.name, v.activated_stake, v.commission, v.epoch_credits, 
                    v.last_vote, v.delinquent, v.root_slot, v.start_epoch, v.url, v.ip, v.latitude, v.longitude, 
                    v.country, v.city, v.version, v.superminority, v.epoch_stats, v.epochs_count, 
