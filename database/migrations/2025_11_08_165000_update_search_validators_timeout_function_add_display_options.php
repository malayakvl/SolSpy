...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement("DROP FUNCTION IF EXISTS data.search_validators_timeout(text, text, bigint, text, integer, integer, bigint, bigint[]);");
        
        // Create the updated function with display options
        DB::statement("
            CREATE OR REPLACE FUNCTION data.search_validators_timeout(
                p_search_term text, 
                p_filter_type text, 
                p_user_id bigint, 
                p_sort_column text, 
                p_offset integer, 
                p_limit integer,
                p_validator_id bigint DEFAULT NULL::bigint,
                p_validator_ids bigint[] DEFAULT NULL::bigint[],
                p_only_with_name boolean DEFAULT NULL,
                p_only_with_website boolean DEFAULT NULL,
                p_not_russian boolean DEFAULT NULL,
                p_only_validated boolean DEFAULT NULL,
                p_only_with_mev_and_zero_commission boolean DEFAULT NULL
            )
            RETURNS TABLE(
                id bigint, 
                vote_pubkey character varying, 
                node_pubkey character varying, 
                ...
                jito boolean, 
                jito_commission double precision, 
                avatar_file_url character varying, 
                software_client character varying, 
                has_screenshot boolean, 
                updated_at timestamp without time zone, 
                is_hightlighted_old boolean, 
                is_top_old boolean, 
                is_highlighted boolean, 
                is_top boolean, 
                epoch_credits_history jsonb, 
                epoch_stats_history jsonb, 
                epoch_credits_history_dump jsonb, 
                sfdp_status character varying, 
                spy_rank numeric, 
                ...
                jiito_score double precision, 
                jiito_score_voter double precision, 
                jiito_score_validator double precision, 
                is_favorite bigint, 
                is_compare bigint, 
                is_blocked bigint, 
                is_notice bigint,
                country_iso character varying, 
                country_iso3 character varying, 
	...
	' from COALESCE(v.country, ''))) = lower(trim(both ' 
	' from COALESCE(c.name, '')))
                ...
                WHERE 
                    -- Only apply validator ID filter if p_validator_id is provided
                    (p_validator_id IS NULL OR v.id = p_validator_id)
                    -- Apply validator IDs array filter if provided
                    AND (p_validator_ids IS NULL OR cardinality(p_validator_ids) = 0 OR v.id = ANY(p_validator_ids))
                    AND (p_search_term IS NULL OR p_search_term = '' OR 
                     lower(trim(both ' 
	' from COALESCE(v.name, ''))) ILIKE ('%' || lower(trim(both ' 
	' from COALESCE(p_search_term, ''))) || '%') 
                     OR lower(trim(both ' 
	' from COALESCE(v.vote_pubkey, ''))) ILIKE ('%' || lower(trim(both ' 
	' from COALESCE(p_search_term, ''))) || '%'))
                    -- Add display options filters
                    AND (p_only_with_name IS NULL OR p_only_with_name = false OR (v.name IS NOT NULL AND trim(v.name) != ''))
                    AND (p_only_with_website IS NULL OR p_only_with_website = false OR (v.url IS NOT NULL AND trim(v.url) != ''))
                    AND (p_not_russian IS NULL OR p_not_russian = false OR (v.country IS NULL OR trim(v.country) = '' OR lower(trim(v.country)) NOT IN ('russia', 'российская федерация', 'россия', 'rf')))
                    AND (p_only_validated IS NULL OR p_only_validated = false OR (v.delinquent = false))
                    AND (p_only_with_mev_and_zero_commission IS NULL OR p_only_with_mev_and_zero_commission = false OR (v.jito = true AND v.commission = 0))
                GROUP BY 
    ...
    public function down(): void
    {
        ...
        DB::statement("DROP FUNCTION IF EXISTS data.search_validators_timeout(text, text, bigint, text, integer, integer, bigint, bigint[], boolean, boolean, boolean, boolean, boolean);");
        
        // Recreate the original function without display options
        DB::statement("
            CREATE OR REPLACE FUNCTION data.search_validators_timeout(
                p_search_term text, 
                p_filter_type text, 
