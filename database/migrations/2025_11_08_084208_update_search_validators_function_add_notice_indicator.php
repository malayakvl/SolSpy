...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Create the new function with the notice indicator
        DB::statement("
            CREATE FUNCTION data.search_validators_timeout(p_search_term text, p_filter_type text, p_user_id bigint, p_sort_column text, p_offset integer, p_limit integer, p_validator_id bigint DEFAULT NULL::bigint, p_validator_ids bigint[] DEFAULT NULL::bigint[])
            ...
            BEGIN
                ...
                WHERE 
                    ...
                    AND (p_filter_type IS NULL OR p_filter_type = '' OR p_filter_type = 'all' 
                         ...
                         OR (p_filter_type = 'notice' AND v2u.id IS NOT NULL AND v2u.type = 'notice'))
                GROUP BY 
    ...
    {
        // Drop the function with notice indicator
        DB::statement("DROP FUNCTION IF EXISTS data.search_validators_timeout(text, text, bigint, text, integer, integer, bigint, bigint[]);");
        
        // Recreate the previous version without the notice indicator
        DB::statement("
            CREATE FUNCTION data.search_validators_timeout(p_search_term text, p_filter_type text, p_user_id bigint, p_sort_column text, p_offset integer, p_limit integer, p_validator_id bigint DEFAULT NULL::bigint, p_validator_ids bigint[] DEFAULT NULL::bigint[])
