...
class ValidatorService
{
    ...
    public function getValidators($userId, string $filterType, int $offset, int $limit, $totalStakeLamports, $sortColumn = 'spyRank', $searchTerm = '', $displayOptions = [])
    {
...
//            $validator->latestVersion = $latestScore ? $latestScore->version : null;
    ...
    public function getTimeoutData($sortColumn, $sortDirection, $totalStakeLamports, $userId = null, $filterType = 'all', $limit = 10, $offset = 0, $searchTerm = '', $validatorId = null, $displayOptions = [])
    {
        // Extract display options with defaults
        $onlyWithName = $displayOptions['onlyWithName'] ?? null;
        $onlyWithWebsite = $displayOptions['onlyWithWebsite'] ?? null;
        ...
        // Подготовка массива для p_validator_ids (если передан единичный $validatorId)
        $validatorIdsPgParam = null;
        if (!empty($validatorId)) {
            $validatorIdsPgParam = '{' . (int)$validatorId . '}'; // Формат массива для Postgres: {123}
        }

        // Безопасный запрос с именованными плейсхолдерами PDO
        $queryNew = "SELECT * FROM data.get_validators_with_timeout(
        ...
        p_offset => :p_offset,
        p_limit => :p_limit,
        p_only_with_name => :p_only_with_name,
        p_only_with_website => :p_only_with_website,
        p_not_russian => :p_not_russian,
        p_only_validated => :p_only_validated,
        p_only_with_mev_and_zero_commission => :p_only_with_mev_and_zero_commission,
        p_validator_ids => :p_validator_ids
    );";

        // Выполняем SQL и безопасно подставляем типы данных
        $validatorsData = DB::select($queryNew, [
            'p_search_term'                       => $searchTerm ?: null,
            'p_filter_type'                       => $filterType ?: 'all',
            'p_user_id'                           => $userId ? (int)$userId : null,
            'p_sort_column'                       => $sortColumn ?: 'id',
            'p_offset'                            => (int)$offset,
            'p_limit'                             => (int)$limit,
            'p_only_with_name'                    => is_null($onlyWithName) ? null : (bool)$onlyWithName,
            'p_only_with_website'                 => is_null($onlyWithWebsite) ? null : (bool)$onlyWithWebsite,
            'p_not_russian'                       => is_null($notRussian) ? null : (bool)$notRussian,
            'p_only_validated'                    => is_null($onlyValidated) ? null : (bool)$onlyValidated,
            'p_only_with_mev_and_zero_commission' => is_null($onlyWithMevAndZeroCommission) ? null : (bool)$onlyWithMevAndZeroCommission,
            'p_validator_ids'                     => $validatorIdsPgParam,
        ]);
    ...
    public function _getTimeoutData($sortColumn, $sortDirection, $totalStakeLamports, $userId = null, $filterType = 'all', $limit = 10, $offset = 0, $searchTerm = '', $validatorId = null, $displayOptions = [])
    {
        // Extract display options with defaults
        $onlyWithName = $displayOptions['onlyWithName'] ?? null;
        $onlyWithWebsite = $displayOptions['onlyWithWebsite'] ?? null;
...
//        if ($userId)
//            if ($validatorId)
//                $queryNew = "SELECT * FROM data.get_validators_with_timeout('" .$searchTerm. "', '" .$filterType. "', $userId, '" .$sortColumn. "', $offset, $limit, $validatorId, NULL, " . ($onlyWithName === true ? 'true' : ($onlyWithName === false ? 'false' : 'null')) . ", " . ($onlyWithWebsite === true ? 'true' : ($onlyWithWebsite === false ? 'false' : 'null')) . ", " . ($notRussian === true ? 'true' : ($notRussian === false ? 'false' : 'null')) . ", " . ($onlyValidated === true ? 'true' : ($onlyValidated === false ? 'false' : 'null')) . ", " . ($onlyWithMevAndZeroCommission === true ? 'true' : ($onlyWithMevAndZeroCommission === false ? 'false' : 'null')) . ");";
//            else
//                $queryNew = "SELECT * FROM data.get_validators_with_timeout('" .$searchTerm. "', '" .$filterType. "', $userId, '" .$sortColumn. "', $offset, $limit, null, NULL, " . ($onlyWithName === true ? 'true' : ($onlyWithName === false ? 'false' : 'null')) . ", " . ($onlyWithWebsite === true ? 'true' : ($onlyWithWebsite === false ? 'false' : 'null')) . ", " . ($notRussian === true ? 'true' : ($notRussian === false ? 'false' : 'null')) . ", " . ($onlyValidated === true ? 'true' : ($onlyValidated === false ? 'false' : 'null')) . ", " . ($onlyWithMevAndZeroCommission === true ? 'true' : ($onlyWithMevAndZeroCommission === false ? 'false' : 'null')) . ");";
//        else
//            if ($validatorId)
//                $queryNew = "SELECT * FROM data.get_validators_with_timeout('" .$searchTerm. "', '" .$filterType. "', null, '" .$sortColumn. "', $offset, $limit, $validatorId, NULL, " . ($onlyWithName === true ? 'true' : ($onlyWithName === false ? 'false' : 'null')) . ", " . ($onlyWithWebsite === true ? 'true' : ($onlyWithWebsite === false ? 'false' : 'null')) . ", " . ($notRussian === true ? 'true' : ($notRussian === false ? 'false' : 'null')) . ", " . ($onlyValidated === true ? 'true' : ($onlyValidated === false ? 'false' : 'null')) . ", " . ($onlyWithMevAndZeroCommission === true ? 'true' : ($onlyWithMevAndZeroCommission === false ? 'false' : 'null')) . ");";
//            else {
...
//            }
        ...
        if ($searchTerm !== null && $searchTerm !== '') {
            $params[] = "p_search_term => " . pg_escape_literal($searchTerm);
        }
        if ($filterType !== null) {
            $params[] = "p_filter_type => " . pg_escape_literal($filterType);
        }
        if ($sortColumn !== null) {
            $params[] = "p_sort_column => " . pg_escape_literal($sortColumn);
        }
        $params[] = "p_offset => " . (int)$offset;
        $params[] = "p_limit => " . (int)$limit;

        if ($onlyWithName !== null) {
            $params[] = "p_only_with_name => " . ($onlyWithName ? 'true' : 'false');
        }
        if ($onlyWithWebsite !== null) {
            $params[] = "p_only_with_website => " . ($onlyWithWebsite ? 'true' : 'false');
        }
        if ($notRussian !== null) {
            $params[] = "p_not_russian => " . ($notRussian ? 'true' : 'false');
        }
        if ($onlyValidated !== null) {
            $params[] = "p_only_validated => " . ($onlyValidated ? 'true' : 'false');
        }
        if ($onlyWithMevAndZeroCommission !== null) {
            $params[] = "p_only_with_mev_and_zero_commission => " . ($onlyWithMevAndZeroCommission ? 'true' : 'false');
        }

// Если есть массив валидаторов:
        if (!empty($validatorIds)) {
            $params[] = "p_validator_ids => ARRAY[" . implode(',', array_map('intval', $validatorIds)) . "]::bigint[]";
        }

        $queryNew = "SELECT * FROM data.get_validators_with_timeout(" . implode(', ', $params) . ");";
dd($queryNew);exit;
        $validatorsData = DB::select($queryNew);
    ...
    public function fetchDataTopValidators($sortedValidators, $totalStakeLamports) {
        ...
        //     ->join('data.validator_scores', 'data.validator_scores.vote_pubkey', '=', 'data.validators.vote_pubkey')
    ...
    public function fetchDataNoticeValidators($userId, string $filterType, int $offset, $totalStakeLamports, $favoriteIds = null)
    {
        // For authenticated users, use the favorites table
        if ($userId)
            $queryNew = "SELECT * FROM data.search_validators('', 'notice', ".$userId.", 'spy_rank', 0, 10);";
        else                
        ...
        $validatorsAllData = DB::table('data.validators')
            ->orderBy('activated_stake', 'DESC')->get();
        $sortedValidators = $validatorsAllData->toArray(); 
        $filteredTotalCount = count($validatorsData);
    ...
    public function timeoutNoticeData($sortColumn, $sortDirection, $totalStakeLamports, $userId = null, $filterType = 'all', $limit = 10, $offset = 0, $searchTerm = '', $favoriteIds = null)
    { 
        if ($userId) {
            $queryNew = "SELECT * FROM data.search_validators_timeout('" . $searchTerm . "', 'notice', $userId, '" . $sortColumn . "', $offset, $limit, null);";
        } else {
            // For unauthenticated users with specific favorite IDs, we need to use a different approach
        ...
        return [
            'validatorsData' => $validatorsData,
            'totalCount' => count($validatorsData),
            'filteredTotalCount' => count($validatorsData),
            'totalStakeLamports' => $totalStakeLamports,
        ];
    }
    ...
    { 
        if ($userId) {
            ...
            // For unauthenticated users with specific favorite IDs, we need to use a different approach
            // since the search_validators_timeout function doesn't support array parameters
            $queryNew = "SELECT * FROM data.search_validators_timeout('" . $searchTerm . "', '" . $filterType . "', null, '" . $sortColumn . "', $offset, $limit, null, ARRAY[" . implode(',', array_map('intval', $comparisonIds)) . "]);";
        }
        $validatorsData = DB::select($queryNew);
    ...
    public function fetchDataComparisonsValidators1($userId, string $filterType, int $offset, $totalStakeLamports, $comparisonIds = null)
    {
        ...
        if ($userId) {
            ...
            // Calculate total count based on filter
            $totalCountQuery = DB::table('data.validators')
                ->join('data.validators_compare', function($join) use ($userId) {
                    $join->on('data.validators.id', '=', 'data.validators_compare.validator_id')
                         ->where('data.validators_compare.user_id', '=', $userId);
                });
        ...
        // For unauthenticated users with specific comparison IDs
        elseif (!empty($comparisonIds)) {
            // For unauthenticated users with specific comparison IDs, we need to use a different approach
            // since the search_validators function doesn't support array parameters
            $validatorIds = array_map('intval', $comparisonIds);
            $query = DB::select("SELECT * FROM data.search_validators('', 'all', NULL, 'spy_rank', " .$offset. ", 10);");

            // Filter the results by the specific validator IDs
            $allValidators = collect($query);
            $validatorsData = $allValidators->filter(function ($validator) use ($validatorIds) {
                return in_array($validator->id, $validatorIds);
            });
            $validatorsData = collect($query);
            // Calculate total count based on filter
            $filteredTotalCount = DB::table('data.validators')
        ...
        // For unauthenticated users without comparison IDs, return empty results
        else {
            $validatorsData = collect([]); // Empty collection
            $filteredTotalCount = 0;
        }
