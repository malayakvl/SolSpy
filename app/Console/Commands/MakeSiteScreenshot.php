...
//            ->get();
        $validators = Validator::where('id', '=', 18789)->orderBy('id')
            ->get();
        foreach ($validators as $validator) {
            // Skip validators without a URL
            if (empty($validator->url)) {
                echo "Skipping validator " . $validator->id . " - no URL provided\n";
                continue;
            }
            ...
            try {
                ...
                $page->navigate($validator->url)->waitForNavigation();
                ...
                // Update validator has_screenshot to true
                $validator->has_screenshot = true;
                $validator->save();
