...
class UpdateMetricsFromServer extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'score:update-metrics-from-server';
     ...
     * @var string
     */
    protected $description = 'Update metrics from server';
    ...
    public function handle()
    {
        ...
        try {
            ...
            // 1. Выполняем команду с флагом --output json
            $command = "{$solanaPath} validators -um --sort=credits --output json";
//            $command = "{$solanaPath} validators -um --sort=credits -r -n | grep -e 53RJBy7aBGA7Aag6AryxEmBbsHDgwfBWagLrPbGHnfvR";
            $output = $ssh->exec($command);

            if (empty($output)) {
                $this->error('Команда не вернула данные');
                return 1;
            }

            // 2. ДЕКОДИРУЕМ JSON (эту строчку случайно пропустили)
            $data = json_decode($output, true);

            if (!$data || !isset($data['validators'])) {
                $this->error('Не удалось распарсить JSON от Solana CLI');
                return 1;
            }

            // 3. СОРТИРУЕМ ВРУЧНУЮ ПО epochCredits (ПО УБЫВАНИЮ)
            usort($data['validators'], function ($a, $b) {
            ...
            // 4. Проставляем честный ранг после сортировки
            foreach ($data['validators'] as $index => &$validator) {
...
//                if (($validator['voteAccountPubkey'] ?? '') === $myVotePubkey) {
...
//
//            if ($myValidator) {
//                $this->info("Валидатор найден! Ранг: {$myValidator['rank']}"); // Выдаст честные 134
//                $this->line("Кредиты эпохи: " . ($myValidator['epochCredits'] ?? 'N/A'));
//                $this->line("Повні дані: " . json_encode($myValidator, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
//            } else {
//                $this->error("Валидатор {$myVotePubkey} не найден в списке");
//            }
            exit;
